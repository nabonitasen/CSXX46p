# agent_code/ppo_rule_based/train.py
"""
Training module for Sequential PPO → Rule-Based Agent

This module handles:
1. PPO updates based on rule-based agent's final action choices
2. Reward shaping for PPO component (same as base PPO)
3. Model persistence and metrics tracking
4. Rule-based agent acts as final decision layer (trains PPO to align with rule-based choices)
"""
...
try:
    from . import callbacks as ppo_rule_callbacks
except Exception:
    import callbacks as ppo_rule_callbacks
...
def setup_training(self):
    ...
    """
    self.name = "PPO→Rule Training"
    self.logger.info("Setting up PPO→Rule-Based training.")
    ...
    self.logger.info("PPO→Rule-Based training setup complete.")
    self.logger.info(f"Sequential flow: PPO suggests → Rule-based decides → PPO learns")
...
def game_events_occurred(self, old_game_state: dict, self_action: str,
    ...
    NOTE: The action here is the FINAL action (from rule-based agent),
    and we train PPO on this action. This helps PPO learn from rule-based decisions.
    """
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    try:
        ...
        self.logger.info(f"Model checkpoint saved to '{MODEL_PATH}' (Round {last_game_state.get('round', 'N/A')})")
    except Exception as exc:
...
# ===================================================================
# HELPER FUNCTIONS (imported from base PPO train.py)
# ===================================================================

def count_crates_in_bomb_range(game_state, bomb_pos):
    """Count crates within bomb blast radius."""
    ...
    for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
        for i in range(1, 4):  # Blast radius of 3
...
def is_approaching_target(old_pos, new_pos, targets):
    """Check if new position is closer to any target than old position."""
    if not targets:
        return False

    old_distances = [abs(old_pos[0] - tx) + abs(old_pos[1] - ty) for tx, ty in targets]
    new_distances = [abs(new_pos[0] - tx) + abs(new_pos[1] - ty) for tx, ty in targets]

    old_min_dist = min(old_distances)
    new_min_dist = min(new_distances)
