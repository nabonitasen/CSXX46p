"""
Training module for Sequential Q-Learning → LLM Agent
...
5. LLM acts as final decision layer (not trained, just inference)
"""
...
def setup_training(self):
    ...
    self.name = "Hybrid Q-LLM Training"
    ...
    # Epsilon decay stages (moderate exploration with LLM guidance)
    self.epsilon_stages = [
    ...
    self.total_reward = 0
    self.round_rewards = []
    self.llm_agreement_count = 0  # Track how often we agree with LLM
    self.llm_disagreement_count = 0

    # === LLM interaction settings ===
    self.use_llm_for_training = False  # DISABLED for fair comparison with q_learning
...
def game_events_occurred(self, old_game_state: dict, self_action: str,
    ...
    # === LLM-guided reward augmentation ===
    if self.use_llm_for_training:
        llm_bonus = get_llm_alignment_bonus(self, old_game_state, self_action)
        if llm_bonus != 0:
            reward += llm_bonus
            self.logger.debug(f"LLM alignment bonus: {llm_bonus:.2f}")
...
def end_of_round(self, last_game_state: dict, last_action: str, events: list):
    ...
    if hasattr(self, 'llm_agreement_count'):
        total_llm_interactions = self.llm_agreement_count + self.llm_disagreement_count
        if total_llm_interactions > 0:
            agreement_rate = 100.0 * self.llm_agreement_count / total_llm_interactions
            self.logger.info(f"LLM agreement rate: {agreement_rate:.1f}% ({self.llm_agreement_count}/{total_llm_interactions})")
...
def reward_from_events(self, events: List) -> float:
    ...
    game_rewards = {
        ...
        # LLM ALIGNMENT (q_llm specific - not in q_learning)
        "AGREED_WITH_LLM": 0.0,           # DISABLED for fair comparison
        "DISAGREED_WITH_LLM": 0.0,        # DISABLED for fair comparison
    }
...
# ===================================================================
# LLM ALIGNMENT TRACKING
# ===================================================================

def get_llm_alignment_bonus(self, game_state, self_action) -> float:
    """
    Check if action agrees with LLM recommendation and provide bonus.
    Returns reward bonus (positive if agreement, 0 otherwise).
    """
    if not self.use_llm_for_training:
        return 0.0

    try:
        # Get LLM recommendation (with timeout to avoid blocking training)
        llm_context = get_llm_strategic_context(self, game_state)

        if not llm_context.get('llm_available'):
            return 0.0

        llm_action = llm_context.get('recommended_action')

        if llm_action == self_action:
            self.llm_agreement_count += 1
            return self.llm_reward_bonus
        else:
            self.llm_disagreement_count += 1
            return 0.0

    except Exception as e:
        self.logger.debug(f"LLM alignment check failed: {e}")
        return 0.0
