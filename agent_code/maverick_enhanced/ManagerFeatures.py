...
def state_to_features(self, game_state: dict) -> torch.tensor:
    '''
    converts game_state into 26 features (enhanced from 23)
    ...
    returns: torch tensor with shape (1,26)
    '''
    ...
    # Feature 1: Escape time remaining (urgency of current situation)
    # How many steps until we MUST start escaping from current danger
    current_danger_timer = future_explosion_map[player_pos[0], player_pos[1]]
    if current_danger_timer == 1:  # No immediate danger
        escape_urgency = 0.0
    else:
        # Negative values indicate bomb countdown (-3 = about to explode, -2, -1, 0)
        # Convert to urgency score: more negative = more urgent
        escape_urgency = max(0, -(current_danger_timer + 3)) / 4.0  # Normalize to 0-1
    features = np.append(features, escape_urgency)

    # Feature 2: Safe zone accessibility (average distance to safety in all valid directions)
    # Measures how trapped we are - lower is better
    safe_distances = []
    for direction_idx in range(4):
        next_pos = player_pos + STEP[direction_idx]
        # Check if this direction leads to valid space
        if field[next_pos[0], next_pos[1]] == 0:
            # Check if moving this direction gets us closer to safety
            if future_explosion_map[next_pos[0], next_pos[1]] == 1:
                safe_distances.append(1)  # Immediately safe
            elif future_explosion_map[next_pos[0], next_pos[1]] > current_danger_timer:
                safe_distances.append(2)  # Less dangerous
            else:
                safe_distances.append(5)  # Still dangerous
        else:
            safe_distances.append(10)  # Blocked

    avg_safe_distance = np.mean(safe_distances) if safe_distances else 10
    # Invert so higher value = better (closer to safety)
    safe_zone_access = 1.0 / avg_safe_distance
    features = np.append(features, safe_zone_access)

    # Feature 3: Multi-step survival probability (can we survive next 3 moves?)
    # Combines current danger with available escape routes
    survival_score = 0.0
    if current_danger_timer == 1:
        # Currently safe
        survival_score = 1.0
    else:
        # Check if we have at least one escape route that doesn't lead to certain death
        has_escape = False
        for direction_idx in range(4):
            next_pos = player_pos + STEP[direction_idx]
            if field[next_pos[0], next_pos[1]] == 0:  # Valid move
                if not certain_death(next_pos, future_explosion_map, turns=1, forbidden_fields=[player_pos.tolist()]):
                    has_escape = True
                    break

        if has_escape:
            # We have escape options, score based on how much time we have
            survival_score = min(1.0, abs(current_danger_timer) / 4.0)
        else:
            # No clear escape route
            survival_score = -1.0

    features = np.append(features, survival_score)

    # === END NEW SAFETY FEATURES ===
