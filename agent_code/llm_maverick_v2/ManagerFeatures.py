...
def state_to_features(self, game_state: dict, llm_action: str = None) -> torch.tensor:
    '''
    converts game_state into 29 features (23 Maverick + 6 LLM one-hot)

    OPTION 2: Includes LLM action as additional feature
    ...
    :param llm_action: LLM's recommended action (optional, for Option 2)

    returns: torch tensor with shape (1,29)
    '''
    ...
    # Create torch tensor from numpy features (23 features)
    maverick_features = torch.from_numpy(features).float()

    # OPTION 2: Add LLM action as one-hot encoding (6 additional features)
    llm_one_hot = torch.zeros(6)  # Default: no LLM recommendation
    if llm_action is not None and llm_action in ACTIONS:
        action_index = ACTIONS.index(llm_action)
        llm_one_hot[action_index] = 1.0

    # Concatenate: [23 Maverick features + 6 LLM one-hot] = 29 total
    combined_features = torch.cat([maverick_features, llm_one_hot])

    return combined_features.unsqueeze(0)  # Shape: (1, 29)